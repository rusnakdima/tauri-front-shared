...
export async function renderAngularPages(
  ...
  for (const page of context.pages) {
    ...
    const componentHtml = env.render(
      "angular/page.component.html.nj",
      pageContext,
    );
    const componentCss = env.render(
      "angular/page.component.css.nj",
      pageContext,
    );

    await writeFile(
      path.join(pagePath, `${page.kebabName}.component.ts`),
      componentTs,
      dryRun,
    );
    await writeFile(
      path.join(pagePath, `${page.kebabName}.component.html`),
      componentHtml,
      dryRun,
    );
    await writeFile(
      path.join(pagePath, `${page.kebabName}.component.css`),
      componentCss,
      dryRun,
    );
  }
...
export async function renderAngularRoutes(
  ...
  await writeFile(
    path.join(routesPath, "app.routes.ts"),
    routesContent,
    dryRun,
  );
