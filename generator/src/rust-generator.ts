...
function createEnv(templateDir?: string): nunjucks.Environment {
  const dirs = templateDir
    ? [templateDir, path.join(templateDir, 'rust'), path.join(templateDir, 'tauri'), './templates/rust', './templates/rust/rust', './templates/tauri', './templates/tauri/tauri']
    : ['./templates/rust', './templates/rust/rust', './templates/tauri', './templates/tauri/tauri'];
...
export async function renderRustCommands(
  ...
  if (context.entities.length > 0) {
    ...
    await writeFile(path.join(commandsPath, 'mod.rs'), '// No entities defined\n', dryRun);
  }
