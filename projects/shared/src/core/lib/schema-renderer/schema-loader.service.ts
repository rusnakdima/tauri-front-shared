...
export class SchemaLoaderService {
  ...
  async loadSchema(mode: "file" | "cloud" | "embedded", source?: string): Promise<UiSchema> {
    switch (mode) {
      case "embedded":
        return this.loadEmbedded(source || "/assets/schema.json");
      case "file":
        if (!source) throw new Error("File path must be provided for file mode");
        return this.loadFromFile(source);
      case "cloud":
        if (!source) throw new Error("Endpoint must be provided for cloud mode");
  ...
  async loadSchemaLocalFirst(designerEndpoint?: string): Promise<UiSchema | null> {
    const { invoke } = await import("@tauri-apps/api/core");

    try {
      const cached = await invoke<{ status: string; data: UiSchema }>("get_schema_local_first", {
        appId: this.appId,
      });
    ...
    if (designerEndpoint) {
      try {
        const cloud = await invoke<{ status: string; data: UiSchema }>("sync_schema_from_cloud", {
          appId: this.appId,
          designerEndpoint,
        });
        if (cloud?.status === "success" && cloud.data) {
          console.log("[SchemaLoader] Synced from cloud via Designer");
          await invoke("save_schema_local", { appId: this.appId, schema: cloud.data });
          return cloud.data;
        }
      } catch (e) {
  ...
  validateSchema(schema: unknown): ValidationResult {
    ...
    if (!schema || typeof schema !== "object") {
      errors.push({ path: "$", message: "Schema must be an object", severity: "error" });
    ...
    if (typeof s.schemaVersion !== "string") {
      errors.push({ path: ".schemaVersion", message: "Schema must have a schemaVersion string", severity: "error" });
    }

    if (!s.pages || !Array.isArray(s.pages)) {
      errors.push({ path: ".pages", message: "Schema must have a pages array", severity: "error" });
    }

    if (!s.components || !Array.isArray(s.components)) {
      errors.push({ path: ".components", message: "Schema must have a components array", severity: "error" });
    }
